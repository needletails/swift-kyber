...
#if (!defined(OQS_HAVE_POSIX_MEMALIGN) && !defined(__APPLE__)) || defined(__MINGW32__) || defined(__MINGW64__) || defined(_MSC_VER)
#include <malloc.h>
#endif
...
#if defined(OQS_USE_OPENSSL)
	return OSSL_FUNC(OPENSSL_zalloc)(num_elements * element_size);
#else
...
#if defined(OQS_USE_OPENSSL)
    return OSSL_FUNC(OPENSSL_strdup)(str);
#else
